...
[![Watch the video](/thumbnail.png)](https://youtube.com/shorts/4Ov67jIUQEY?feature=share)
...
<p>
Among these changes, Alberto implemented a <i>handle_request</i> function to combine the previously defined fucntions (<i>raw_data_to_pc</i> & <i>get_song_from_pc</i>). This request handler connects to a client and calls <i>collect_pico_data</i> to obtain the values from the Pico then send them to the PC. In this handler, we also have an API Endpoint Routing that utilizes API contracts to send sensor data from the Pico to the PC through a GET request. Afterwards, a POST request would be sent out allowing the client to receive the data and disconnect from the web server. This disconnect was done so that we could have an “idle” state where we could reinitiate the program by pressing a button rather than having to reset the board or turn on/off the power. Overall, this handles the communication between the Pico and the PC while calling other functions to obtain, interpret, or receive data.
</p>
...
At first, all of the project updates were made in the main branch. This was mainly for ease of use and simplicity. Following the first agile sprint, we had an IDR with Ryan, in which he questioned our method of having a singular branch that all the work was done in. He advised us that it would be “right” to have separate branches so we can push to main separately, and that the main branch should be reserved for tested and verified code. Each of the projects’ contributors should have their own branch where they work on developing new features, testing code, or fixing bugs. Once a person is confident that their code works, they can request to merge into the main branch. He also advised that the main code should be more protected, and he suggested a system in which one or two other people have to approve the changes to officially merge the code. 

We followed this approach for the second agile sprint. Each team member created and did their work in their own branch. When they wanted to merge that new work to the main branch, they would submit a pull request and another one of us would review and approve that request.  This workflow not only protected the integrity of the main branch but also improved accountability and made it easier to rollback changes when needed. 

# Testing
Since the pico was provided to us without MicroPython installed, we wanted a way to test that MicroPython was installed correctly. Thus, we ran a script to blink the built-in LED on the Pico [(blink.py)](blink.py/blink.py) to verify MicroPython was installed correctly and that our Pico wasn't broken.

Once all of us had written our code in parallel according to the flowchart from above, testing it was relatively simple as we had defined the inputs and outputs of each function and had an idea of what the outputs were. The only part of this project that we were unfamiliar with going into it was using an API Contract to send data over WiFi. Thus, we decided it would be best to first send a sample test message from Pico to PC; since the GET /health API contract was already in place, Alberto and Justin met up prior to doing the full-project integration to test out WiFi communication (as that was the funcitonalities they were assigned) and modify their functions to meet any API contract changes and bugs as needed.

When it came to integrating the full project, we all got into the Terrier Motorsport workspace (as that was the only place with a router the Pico could connect to), and we developed the main incrementally: we tested the function that came first chronologically in our flow (collect_pico_data), wrote a print statement after the function, and tested it to make sure we were getting the output we expected. If we didn't, we used ChatGPT and the error message to deduce what was going wrong and fixed it. We followed this process until we got the light orchestra working. Since analyzing data structures through print statements while debugging isn't obvious, we wanted an easier way to see if the code was working and, if not, where it crashed. Thus, we went back to design and added a visual indicator, described above.

We ran across many bugs throughout our process of debugging. Here are some of the highlights:
- We tried connecting the Pico and PC to a phone hotspot as the network for communication. It turns out that some phone hotspots have firewalls to block device-device communication on the same network, causing this to not work.
- We set the duty cycle of the buzzer's PWM pin to 0 when we wanted silence, but we never set it to a nonzero value whenever we wanted to play a note as we thought the .freq() method would, keeping the buzzer silent indefinitely.
- We set GPIO pin 12 to be both for the green RBG value for the LED and the PWM pin for the piezo, also keeping the buzzer silent indefinitely. 😅
